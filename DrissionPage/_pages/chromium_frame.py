...
class ChromiumFrame(ChromiumBase):
    ...
    def _get_screenshot(self, path=None, name=None, as_bytes=None, as_base64=None,
        ...
        states = []
        try:
            self._prepare_frame_element_screenshot(ele, states)
            left_top, right_bottom = self._get_ele_screenshot_rect(ele)
            self._ensure_top_viewport_for_screenshot(left_top, right_bottom, states)
            ...
            self._restore_screenshot_states(states)

    def _prepare_frame_element_screenshot(self, ele, states):
        self._save_tab_scroll_state(states)

        target = ele
        for frame in self._get_screenshot_frame_chain():
            self._save_frame_screenshot_state(frame, states)
        ...
        for frame in reversed(self._get_screenshot_frame_chain()):
            frame.frame_ele.scroll.to_see(center=False)
        self._wait_screenshot_chain_stable(ele)
    ...
    def _resize_frame_for_screenshot(self, frame, box):
        ...
        this.style.setProperty('height', Math.ceil(Math.max(r.height, arguments[1] + borderHeight)) + 'px', 'important');''',
                                  width, height)
    ...
    def _ensure_top_viewport_for_screenshot(self, left_top, right_bottom, states):
        if not any(frame._is_diff_domain for frame in self._get_screenshot_frame_chain()):
            return

        viewport_width, viewport_height = self.tab.rect.viewport_size_with_scrollbar
        scroll_x, scroll_y = self.tab.rect.scroll_position
        width = max(viewport_width, ceil(right_bottom[0] - scroll_x))
        height = max(viewport_height, ceil(right_bottom[1] - scroll_y))
        if width <= viewport_width and height <= viewport_height:
            return

        states.append(('device_metrics', self.tab))
        self.tab._run_cdp('Emulation.setDeviceMetricsOverride', width=width, height=height,
                          deviceScaleFactor=self.tab._run_js('return window.devicePixelRatio;'), mobile=False)
        self._wait_animation_frame(self.tab)

    def _save_tab_scroll_state(self, states):
        scroll = self.tab._run_js('return [window.pageXOffset, window.pageYOffset];')
        states.append(('tab_scroll', self.tab, scroll))

    def _save_frame_screenshot_state(self, frame, states):
        style = frame.frame_ele._run_js('return this.getAttribute("style");')
        scroll = frame._run_js('return [window.pageXOffset, window.pageYOffset];')
        states.append(('frame', frame, style, scroll))

    def _restore_screenshot_states(self, states):
        for state in reversed(states):
            kind = state[0]
            try:
                if kind == 'frame':
                    _, frame, style, scroll = state
                    if style is None:
                        frame.frame_ele._run_js('this.removeAttribute("style");')
                    else:
                        frame.frame_ele._run_js('this.setAttribute("style", arguments[0]);', style)
                    frame._run_js('window.scrollTo(arguments[0], arguments[1]);', scroll[0], scroll[1])
                elif kind == 'tab_scroll':
                    _, tab, scroll = state
                    tab._run_js('window.scrollTo(arguments[0], arguments[1]);', scroll[0], scroll[1])
                elif kind == 'device_metrics':
                    _, tab = state
                    tab._run_cdp('Emulation.clearDeviceMetricsOverride', _ignore=True)
    ...
    def _wait_screenshot_chain_stable(self, ele):
        frames = self._get_screenshot_frame_chain()
        frames.append(self.tab)
    ...
    def _mark_frame_elements(self, result):
        if getattr(result, '_type', None) == 'ChromiumElement':
            result._screenshot_frame_owner = self
