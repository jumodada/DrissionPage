...
| `context_get_tabs_as_id` | BrowserContext 标签页查询 | `ChromiumContext.get_tabs(as_id=True)` 运行时还未支持 `as_id` 参数。 |
| `locator_explicit_syntax` | 显式定位符语义 | 显式 `xpath:` / `css:` 未命中时仍可能误走文本兜底，返回文本所在元素。 |
| `object_frame_default` | `<object>` frame-like 元素 | `tag:object` 已是 `ChromiumFrame`，但 `get_frames()` 默认枚举漏掉 object。 |
